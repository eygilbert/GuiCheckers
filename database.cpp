...
int inline TestBoard( CBoard &Board, int nPieces, int np1, int np2, int np3, int RIndex )
{
	...
	for (int stm = 0; stm < 2; stm++)
	{
		...
		if (Board.SideToMove == WHITE) {
			Moves.FindMovesWhite( Board.C ); Win = WHITEWIN; Loss = BLACKWIN;
		} else {
			Moves.FindMovesBlack( Board.C ); Win = BLACKWIN; Loss = WHITEWIN;
		}
				
		if (Moves.numMoves == 0) {SetResult( Index, Loss ); nTotalWins++;} // Can't move, so it's a loss
