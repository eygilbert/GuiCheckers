...
int ComputeIndex(int WS[], int BS[], int &nPieces, int stm, int &bFlip)
{
	...
	if (nPieces == 2) {
		for (i = 0; i < 4; i++)
			if (WS[0] == PC2[0 + i * 4] && BS[0] == PC2[0 + i * 4 + 1])
				break;
	...
	if (nPieces == 3) {
		for (i = 0; i < 12; i++) {
			if
			(
				(WS[0] == PC3[0 + i * 4] && WS[1] == PC3[1 + i * 4] && BS[0] == PC3[2 + i * 4]) ||
				(WS[0] == PC3[0 + i * 4] && BS[0] == PC3[1 + i * 4] && BS[1] == PC3[2 + i * 4])
			) break;
		}
	...
	if (nPieces == 4) {
		...
		if (FourIndex[i] < 8)
			...
			Index = FourPc[FourIndex[i] & 7].GetIndex((31 - Sqs[2]) + (31 - Sqs[3]) * 32,
													  (31 - Sqs[0]) + (31 - Sqs[1]) * 32,
													  stm ^ 1);
			bFlip = 1;
		}
...
//
int GetIndexFromBoard(const CBoard &Board, int &bFlip, int &nPieces)
{
...
//
// This test a board setup, and stores the result
...
int inline TestBoard(CBoard &Board, int nPieces, int np1, int np2, int np3, int RIndex)
{
	...
	for (int stm = 0; stm < 2; stm++) {
		...
		if (GetResult(pResults, Index) != DRAW) {
			nTotalWins++;
			continue;
		}
...
void GenDatabase(int Piece1, int Piece2, int Piece3, int Piece4, int RIndex)
{
	...
	while (nTotalWins != nLastTotalWins) {

		// Keep going until we reach a plateau
...
void InitializeGuiDatabases(SDatabaseInfo &dbInfo)
{
	...
	if (bCheckerBoard) {
		...
		if (FP)
			...
			strcpy(sAFile, "database.jef");
	}
	else
		strcpy(sAFile, "database.jef");

	if (uncompressFileFromArchive(sAFile, "2pc.cdb", ResultsTwo) != 1)
		dbInfo.loaded = false;
	if (uncompressFileFromArchive(sAFile, "3pc.cdb", ResultsThree) != 1)
		dbInfo.loaded = false;
	if (uncompressFileFromArchive(sAFile, "4pc.cdb", ResultsFour) != 1)
		dbInfo.loaded = false;
...
void GenAllDatabases()
{
	ComputeAllIndices();
	for (int i = 0; i < 4; i++)
		GenDatabase(PC2[i * 4], PC2[i * 4 + 1], PC2[i * 4 + 2], PC2[i * 4 + 3], i);
	for (int i = 0; i < 12; i++)
		GenDatabase(PC3[i * 4], PC3[i * 4 + 1], PC3[i * 4 + 2], PC3[i * 4 + 3], ThreeIndex[i]);
	for (int i = 0; i < 9; i++)
		GenDatabase(PC4[i * 4], PC4[i * 4 + 1], PC4[i * 4 + 2], PC4[i * 4 + 3], FourIndex[i]);
	SaveAllDatabases();
}
