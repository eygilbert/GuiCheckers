...
void RunningDisplay(int bestMove, int bSearching)
{
	...
	if (seconds > 0.0f) nps = int(float(nodes) / (1000.0 * seconds)); else nps = 0;
...
const char* GetInfoString( )
{
	...
	{
		sprintf( displayString, "Database : %d-man %s\n", g_dbInfo.numPieces, g_dbInfo.type == DB_WIN_LOSS_DRAW ? "Win/Loss/Draw" : "Perfect Play"  );
	}
	if (pBook)
...
int WINAPI getmove(int board[8][8], int color, double maxtime, char str[1024], int *playnow, int info, int moreinfo, struct CBmove *move)
{
	...
	if (incremental) {
		...
		log("incr %.1f, remaining %.3f, abs maxt %.3f, desired %.3f, new iter maxt %.3f, actual %.3f, margin %.3f %s\n",
			increment, remaining, fMaxSeconds, desired, new_iter_maxtime, 
			elapsed, remaining - elapsed, tag);
	}
#endif
	for (i = 0; i < 64; i++)
		if (BoardLoc[i] >= 0) 
